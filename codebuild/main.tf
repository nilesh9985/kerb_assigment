...
resource "aws_codebuild_project" "KREB" {
  name          = "KREB"
  build_timeout = 10
  badge_enabled = true
  ...
  source {
    type                = "GITHUB"
    location            = "https://github.com/repo_location.git"
    git_clone_depth     = 1
...
output "codebuild-badge" {
  value = aws_codebuild_project.KREB.badge_url
}

resource "aws_codebuild_webhook" "github" {
  project_name = aws_codebuild_project.KREB.name
}
